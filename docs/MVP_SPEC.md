...
| US-04 | 有成年子女的高齡父母 | 簡易更新保單內容並即時通知家庭成員 | 通知列表顯示保單更新提醒 | 🔶 部分（通知展示，無實際編輯串接） |
...
│   │   ├── 家庭保險健康分級入口（查看／重新填寫問卷）
│   │   ├── 保障缺口（目前／目標分開標示 + 達成率 + 已投保成員）
│   │   ├── 可獲保障支持（理賠金）
│   │   │   ├── 固定保障額(請參照保險公司的標準，此處表達非意外事件的保障，如果沒有，請直接加總所有保障的保額即可，並另外命名)
│   │   │   └── 意外理賠（潛在可理賠的保障，例如：就醫實支實付、車禍等等...）（一個事件一個事件呈現）
│   │   └── 待辦事件（顯示總數 → 連結至「提醒」頁）
│   └── 推薦內容
│       ├── 新婚保障規劃教育內容
│       ├── 買房規劃教育內容
...
│
├── 2. 提醒
│   ├── 通知列表（**最左優先**；有未讀時頁籤右上角紅點；每則未讀卡片右上角紅點）
│   ├── 待辦事件（與「保障成員 > 保障規劃」資料串聯）
│   │   └── 勾選已完成 → 移至「歷史事件」
│   └── 歷史事件
│       ├── 保單更新提醒
│       ├── 保單即將到期提醒
...
│
├── 3. AI 顧問
│   ├── 歡迎引導區（AdvisorWelcome：情境捷徑 + 建議提問）
│   ├── 底部常駐 Dock
│   │   ├── 頁籤切換：對話｜情境模擬
│   │   ├── 對話模式：建議 chip + 文字輸入列
│   │   └── 情境模擬模式：成員 + 年齡 + 情境 → 開始模擬
│   └── 模擬結果 Modal（缺口試算 + AI 推薦保單）
│
├── 5. 個人資料（**僅**右上角頭像進入，非 Tab 頁籤）
│   ├── 登入帳號資訊
│   ├── 個人基本資料
│   ├── 家庭保險健康分級（查看／重新填寫問卷）
│   ├── 我的安全文件庫（可上傳）
...
└── 4. 保障
    └── 保障成員（家庭成員儀表板）
        ├── 成員列表（成員數與頂部導覽列同步）
        ├── 成員詳情
        │   ├── 擁有保單（集保同步 + 自行登載；含台灣各家保險公司商品）
        │   ├── 受益人
        │   ├── 個人待辦（與「提醒 > 待辦事件」同資料源）
        │   ├── 保障規劃（原「事件」功能，生命事件列表 + 新增）
        │   └── 安全文件庫（他人文件僅供瀏覽）
        └── 新增成員
```
...
| Tab | 圖示 | 說明 |
|-----|------|------|
| 總覽 | LayoutDashboard | 首頁，家庭保障全貌 |
| 提醒 | Bell（含未讀 badge） | 待辦 + 歷史 + 通知 |
| AI 顧問 | Bot | 對話 + 情境模擬 |
| 保障 | Shield | 保障成員 |
...
| 待辦日曆 | 週／月／年視圖、紅點標示、點擊日期開啟待辦詳情 Modal | ✅ `TodoCalendarPanel` | |
| 待辦事件入口 | 日曆區「查看全部」導向提醒 > **待辦**子頁籤 | ✅ `navigateToReminders('todos')` | |
...
| 功能 | 規格 | MVP 實作 | 待補 |
|------|------|---------|------|
| 待辦事件列表 | 顯示標題、成員、緊急程度、到期日 | ✅ | |
| 勾選完成 | 移至歷史事件，觸發成功通知 | ✅ | 實際推送通知：______ |
| 歷史事件 | 顯示已完成事項與完成日期 | ✅ | |
| 未讀通知 Badge | 每則未讀卡片容器右上角紅點；「通知」子頁籤有未讀時顯示紅點直至全部已讀 | ✅ | |
| 提醒 Tab Badge | 底部／頂部「提醒」導覽數字 = **僅未讀通知數**，不含待辦 | ✅ | |
| 保單更新提醒 | 「XXX 於 YYYY/MM/DD 更新了保障內容」 | ✅ mock | API 事件來源：______ |
| 保單即將到期提醒 | 到期日前 N 天提醒 | ✅ mock | N = ______ 天 |
| 保單申購進度提醒 | 申購審核狀態 | ✅ mock | |
| 理賠申請進度提醒 | 理賠審核/入帳狀態 | ✅ mock | |
...
| 擁有保單 — 集保同步 | 標示「集保同步」chip；區塊頂部說明：資料串聯保險工會集保，為成員本人登入後自動取得 | ✅ `source=union` | 正式集保 API：______ |
| 擁有保單 — 自行登載 | 「新增」按鈕（樣式同保障規劃）；虛線邊框卡片 +「自行登載」chip；Modal 表單登載 | ✅ `addManualPolicy` | |
| 保單來源區隔 | 集保（teal + Link 圖示）vs 自行登載（sand 虛線 + Pen 圖示）視覺明確區分 | ✅ | |
...
| `source` | 說明 |
|----------|------|
| `union` | 串聯保險工會集保；成員登入後自動同步（mock 預設） |
| `manual` | 使用者自行登載，不與集保同步 |
...
```
id, title, memberId, memberName, policyId?,
...
| 來源 | 目標 | 規則 |
|------|------|------|
| 新增家庭事件 | 提醒 > 待辦事件 | 自動建立同名待辦，`source=event` |
| 成員 > 個人待辦 | 提醒 > 待辦事件 | 同一 `todos` 陣列，依 `memberId` 篩選 |
| 總覽 > 待辦事件 | 提醒頁 | 顯示 `todos.filter(!completed).length` |
...
| 項目 | MVP 實作 | 待補 |
|------|---------|------|
| 保單資料來源 | 集保同步（mock `union`）+ 自行登載（`manual`） | 保險工會集保 API：______ |
...
| 保單來源標示 | 集保同步 chip / 自行登載 chip（保障 > 成員詳情） |
...
| 狀態管理 | React Context（AppContext）；`addManualPolicy` 等成員保單操作 |
| 路由 | Tab 切換（無 URL 路由） |
...
```
legacymap/
├── docs/MVP_SPEC.md              ← 本文件
├── public/
│   ├── app-icon.png              App 圖示（頂部不變）
│   └── hero-mountain-nobg.png    總覽歡迎區山形裝飾
├── src/
│   ├── types/                    型別定義（含 PolicySource、NewPolicyInput）
│   ├── data/mockData.ts          6 位成員 + 擬真保單 mock
│   ├── utils/calculations.ts     缺口／情境模擬／意外理賠分組
│   ├── utils/calendar.ts         待辦日曆工具
│   ├── context/AppContext.tsx    全域狀態
│   ├── hooks/
│   │   ├── useMediaQuery.ts
│   │   ├── useMobileKeyboardOffset.ts
│   │   └── useElementHeight.ts
│   ├── components/
│   │   ├── layout/AppShell.tsx
│   │   ├── common/
│   │   │   ├── BrandLogo.tsx, BrandHeader.tsx
│   │   │   ├── PolicyRecommendationPanel.tsx
│   │   │   └── …
│   │   ├── overview/
│   │   │   ├── OverviewPage.tsx, TodoCalendarPanel.tsx
│   │   │   ├── AccidentPayoutPanel.tsx, GapRecommendationModal.tsx
│   │   ├── advisor/
│   │   │   ├── AdvisorPage.tsx, AdvisorWelcome.tsx
│   │   │   └── ScenarioSimulatorSheet.tsx
│   │   ├── protection/ProtectionPage.tsx
...
| 整合項目 | 優先級 | 端點 / 規格 | 狀態 |
|---------|--------|------------|------|
| 保險工會集保 API | P0 | 成員登入後同步保單（`Policy.source=union`） | ⬜ |
| 南山保單 API | P1 | ______ | ⬜ |
| 使用者認證 (SSO) | P0 | ______ | ⬜ |
| AI / LLM 服務 | P1 | ______ | ⬜ |
...
- [x] 響應式 4-tab 導覽（Compact 底部 / Medium+ 頂部 Pill）
- [x] 內容置中舒適閱讀寬度（720px / 總覽 1120px）
- [x] 總覽：健康度、固定保障額、意外理賠分組（≤5 類）、五類缺口（含重大疾病）、待辦日曆、缺口推薦 Modal
- [x] 提醒：待辦/歷史/通知三分頁，勾選完成
- [x] AI 顧問：歡迎引導 + 底部 Dock（對話｜情境模擬）+ 手機鍵盤推升 + 推薦顧問
- [x] 保障：成員詳情、集保同步／自行登載保單、保障規劃、文件庫瀏覽
...
- [x] 通知未讀 Badge 置於每則通知卡片；accent 綠色系按鈕
- [x] 個人資料獨立頁（頭像進入）、登出示意
- [x] 保障缺口「目前／目標」分開標示
...
| AC-25 | 保障 > 成員保單 | 集保保單顯示「集保同步」；可自行登載並顯示「自行登載」區隔 |
...
| AC-02 | 點擊待辦事件入口 | 導向提醒頁並 landing 在「待辦」子頁籤 |
| AC-22 | 全 Tab 內容寬度 | 與總覽相同 1120px，切換 Tab 無水平位移 |
| AC-23 | 保障缺口達標 | 醫療保障等達 100% 時顯示達標視覺獎勵 |
| AC-03 | 勾選待辦完成 | 移至歷史事件，顯示成功橫幅 |
| AC-04 | 成員詳情新增保障規劃 | 提醒頁出現對應待辦 |
| AC-11 | 點擊右上角頭像 | 進入獨立「個人資料」全頁視圖 |
| AC-12 | 新增成員 | 頂部成員數同步更新 |
| AC-13 | 通知列表未讀項目 | 卡片容器右上角紅點 Badge；「通知」子頁籤同步紅點 |
| AC-20 | 提醒導覽 Badge | 數字僅為未讀通知數 |
| AC-21 | 提醒子頁籤順序 | 通知在最左，預設開啟通知 |
...
| TBD-02 | 保險工會集保 API 規格與測試環境 | ______ | ______ |
